    ...
    public function getValuesHtml()
    {
        ...
        if ($_option->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_RADIO
            ...
            foreach ($_option->getValues() as $_value) {
                ...
                $selectHtml .= '<li>' . '<input style="display:none" type="' . $type . '" class="' . $class . ' ' . $require
					...
					.'<input type="text" class="input-text qty"   value="1"  id="qty_'.$_option->getId().'_'.$count.'" />'
					.'<button onclick="buynow('.$_option->getId().','.$count.')" class="button btn-cart" title="buy now" type="button"><span>Buy Now</span></button>'
					.'</span>';
                if ($_option->getIsRequire()) {
