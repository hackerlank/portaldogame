...
class FacileCheckout_OnestepCheckout_Model_Type_Geo
{
    ...
    public function useShipping($method_code = null)
    {
        ...
        else
        {
            ...
            {
                $method_code = Mage::getStoreConfig('onestepcheckout/general/shipping_method');
                foreach ($rates['items'] as $rate)
                {
                    if ($method_code === $rate['code'])
                    {
    ...
    public function getCheckoutMethod()
    {
        ...
        {
            if (Mage::helper('onestepcheckout')->isGuestCheckoutAllowed())
    ...
    public function saveBilling($data, $cust_addr_id, $validate = true, $skip_save = false)
    {
        ...
        {
			$data['contact'] = $data['contact_select'] .': '. $data['contact'];
		...
		if($skip_save){
			...
			$mage_ver = Mage::helper('onestepcheckout')->getMagentoVersion();
    ...
    protected function _processValidateCustomer(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        if ($this->getQuote()->getCheckoutMethod() == self::REGISTER)
        {
            ...
            $cust_data	= array(
                ...
                'confirmation' => 'confirm_password', // customer_password 和 confirm_password都是沒有的     
                'firstname'    => 'firstname',
                'lastname'     => 'lastname',
                'gender'       => 'gender',
                'taxvat'       => 'taxvat');
    ...
    public function validate()
    {
        ...
        if (!Mage::helper('onestepcheckout')->isGuestCheckoutAllowed() && $quote->getCheckoutMethod() == self::GUEST)
    ...
    public function saveOrder()
    {
        ...
        $service_quote = Mage::getModel('onestepcheckout/service_quote', $this->getQuote());
        ...
        if ($order)
        {        	
			if ( $_POST['giftwrap'] == 'Yes') // Gift Wrap Condition
			{
				...
				$osc_quote_id = Mage::getSingleton('checkout/session')->getQuoteId();
					
				$writeConnection = Mage::getSingleton('core/resource')->getConnection('core_write');
									
				$query_sales_flat_quote="UPDATE `sales_flat_quote` SET `grand_total`= '$base_grand_price_totals', `base_grand_total` = '$base_grand_price_totals' WHERE `entity_id` ='$osc_quote_id'"; // sales_flat_quote table
				$writeConnection->query($query_sales_flat_quote);
								
				$query_sales_flat_quote_address="UPDATE `sales_flat_quote_address` SET `grand_total`= '$base_grand_price_totals', `base_grand_total` = '$base_grand_price_totals' WHERE `quote_id` ='$osc_quote_id'"; // sales_flat_quote_address table
				$writeConnection->query($query_sales_flat_quote_address);
				
				$lo  = $order->getId();
				$query_sales_flat_order="UPDATE `sales_flat_order` SET `grand_total`= '$base_grand_price_totals', `base_grand_total` = '$base_grand_price_totals',`giftwrap` = '1' ,`giftwrap_price` = '$giftwrap_admin_price' WHERE `entity_id` ='$lo'";
				// sales_flat_order table
				$writeConnection->query($query_sales_flat_order);
				
				$query_sales_flat_order_grid="UPDATE `sales_flat_order_grid` SET `grand_total`= '$base_grand_price_totals', `base_grand_total` = '$base_grand_price_totals' WHERE `entity_id` ='$lo'"; // sales_flat_order_grid table
				$writeConnection->query($query_sales_flat_order_grid);
				
				/*$query_sales_flat_quote="UPDATE `sales_flat_order_payment` SET `base_amount_ordered`= '$base_grand_price_totals', `amount_ordered` = '$base_grand_price_totals' WHERE `entity_id` ='$osc_quote_id'"; // sales_flat_order_payment table
				$writeConnection->query($query_sales_flat_quote);*/
    ...
    protected function _prepareNewCustomerQuote()
    {
		...
		$bill_data = Mage::app()->getRequest()->getPost('billing', array()); 
		$cust_bill->addData($bill_data);
        ...
        if($ship)
        {
        	if(!$ship->getSameAsBilling())
        	{
				...
				$ship_data = Mage::app()->getRequest()->getPost('shipping', array()); 
				$cust_ship->addData($ship_data);
